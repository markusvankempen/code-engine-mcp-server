  ...
  try {
    switch (name) {
      case 'ce_validate_dockerfile': {
        ...
        const resolvePath = (p: string) => p.startsWith('~') ? p.replace(/^~/, os.homedir()) : p;
        const dfPath = resolvePath(args.dockerfile_path as string);
        const expectedPort: number = (args.expected_port as number) || 8080;
      ...
      case 'ce_create_build_run': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const body: any = { build_name: args.build_name };
      ...
      case 'ce_create_job': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const body: any = { name: args.name, image_reference: args.image, run_mode: 'task' };
        if (args.scale_array_spec) body.scale_array_spec = args.scale_array_spec;
        if (args.scale_cpu_limit) body.scale_cpu_limit = args.scale_cpu_limit;
        if (args.scale_memory_limit) body.scale_memory_limit = args.scale_memory_limit;
        if (args.env_vars) {
          body.run_env_variables = Object.entries(args.env_vars as Record<string, string>).map(([name, value]) => ({
            type: 'literal', name, value,
          }));
      ...
      case 'ce_create_job_run': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const body: any = { job_name: args.job_name };
        if (args.name) body.name = args.name;
      ...
      case 'ce_list_job_runs': {
        ...
        const { base, headers } = await ceApi(args.project_id as string, token);
        const params: any = {};
        if (args.job_name) params.job_name = args.job_name;
      ...
      case 'ce_list_secrets': {
        ...
        const secrets = (response.data.secrets || []).map((s: any) => ({
      ...
      case 'ce_get_domain_mapping': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const response = await axios.get(`${base}/domain_mappings/${encodeURIComponent(args.domain_name as string)}`, { headers });
        return { content: [{ type: 'text', text: JSON.stringify(response.data, null, 2) }] };
      }
      ...
      case 'ce_create_tls_secret_from_pem': {
        ...
        const certPem = fs.readFileSync(resolvePath(args.cert_pem_path as string), 'utf8');
        const keyPem = fs.readFileSync(resolvePath(args.key_pem_path as string), 'utf8');
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
      ...
      case 'ce_delete_domain_mapping': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        await axios.delete(`${base}/domain_mappings/${encodeURIComponent(args.domain_name as string)}`, { headers });
      ...
      case 'ce_refresh_icr_pull_secret': {
        ...
        const token = await getIAMToken(apiKey);
        const { base, headers } = await ceApi(args.project_id as string, token);
        const secretName = (args.secret_name as string) || 'icr-pull-secret';
        const icrHost = (args.icr_host as string) || 'us.icr.io';
      ...
      case 'ce_wait_for_app_ready': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const maxWait = ((args.timeout_seconds as number) || 120) * 1000;
        const interval = 5000;
        const start = Date.now();
        let last: any = {};
        const pollHistory: Array<{ elapsed_s: number; status: string; reason?: string; revision?: string }> = [];
      ...
      case 'ce_wait_for_build_run': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const maxWait = ((args.timeout_seconds as number) || 600) * 1000;
        const interval = 10000;
        const start = Date.now();
        let last: any = {};
        const pollHistory: Array<{ elapsed_s: number; status: string; reason?: string }> = [];
        while (Date.now() - start < maxWait) {
          const response = await axios.get(`${base}/build_runs/${args.build_run_name}`, { headers });
          last = response.data;
          const snap = {
      ...
      case 'icr_create_namespace': {
        const apiKey = getApiKey();
        const token = await getIAMToken(apiKey);
        const host = (args.region as string) || 'us.icr.io';
        const payload = JSON.parse(Buffer.from(token.split('.')[1], 'base64').toString());
        const accountId: string = payload?.account?.bss || '';
        const response = await axios.put(
          `https://${host}/api/v1/namespaces/${encodeURIComponent(args.namespace as string)}`,
          {},
          { headers: { Authorization: `Bearer ${token}`, Account: accountId, 'Content-Type': 'application/json' } }
        );
        return { content: [{ type: 'text', text: JSON.stringify(response.data, null, 2) }] };
      }
      ...
      case 'proc_build_push_deploy': {
        ...
        const contextPathRaw = args.context_path as string;
        const fsM = await import('fs');
        const pathM = await import('path');
        const osM = await import('os');
        const resolveProcPath = (p: string) => p.startsWith('~') ? p.replace(/^~/, osM.homedir()) : p;
        ...
        if (fsM.existsSync(dockerfilePath)) {
          ...
          const expectedPort: number = (args.port as number) || 8080;
          ...
          if (wrongArch.length > 0) valErrors.push(`Architecture: FROM --platform must be linux/amd64. Found: ${wrongArch.map(l => l.trim()).join(' | ')}`);
          ...
          if (exposedPorts.includes(80)) valErrors.push('Port 80 is not allowed in Code Engine. Use port 8080.');
        ...
        // 1) detect runtime
        let runtime = 'podman';
        try { await execFileAsync('podman', ['--version']); } catch { runtime = 'docker'; }
        steps.push(`[1/5] Using container runtime: ${runtime}`);

        // 2) resolve project
        const token1 = await getIAMToken(getApiKey());
        const projectId1 = await resolveProjectId(args.project_id_or_name as string, token1);
        steps.push(`[2/5] Resolved project: ${projectId1}`);

        // 3) build the image name from namespace + app + tag
        const icrHost = (args.icr_host as string) || 'us.icr.io';
        const imageTag = (args.image_tag as string) || 'latest';
        ...
        const pullSecretName = (args.image_secret as string) || 'icr-pull-secret';
        try {
          try { await axios.delete(`${base1}/secrets/${pullSecretName}`, { headers: headers1 }); } catch (e: any) { if (e.response?.status !== 404) throw e; }
        ...
        const appPayload1: any = {
          image_reference: imageName,
          image_secret: args.image_secret,
          scale_initial_instances: 1,
          scale_min_instances: (args.scale_min_instances as number) ?? 0,
          scale_max_instances: (args.scale_max_instances as number) ?? 10,
          image_port: (args.port as number) ?? 8080,
        };
        if (args.env_vars) {
          appPayload1.run_env_variables = Object.entries(args.env_vars as Record<string, string>).map(([k, v]) => ({ type: 'literal', name: k, value: v }));
        }
        let appEndpoint1 = '';
        try {
        ...
        // wait for ready
        const maxWait1 = ((args.timeout_seconds as number) || 180) * 1000;
        const start1 = Date.now();
        let last1: any = {};
        const pollHistory1: Array<{ elapsed_s: number; status: string; reason?: string; revision?: string }> = [];
        while (Date.now() - start1 < maxWait1) {
          ...
          if (pollHistory1.length === 0 || pollHistory1[pollHistory1.length - 1].status !== snap1.status) pollHistory1.push(snap1);
          if (last1.status === 'ready') break;
      ...
      case 'proc_setup_custom_domain': {
        ...
        const projectId2 = await resolveProjectId(args.project_id_or_name as string, token2);
      ...
      case 'proc_build_run_and_deploy': {
        ...
        const projectId3 = await resolveProjectId(args.project_id_or_name as string, token3);
        ...
        const outputImage: string = buildConfig.data.output_image;
        ...
        const buildRunName: string = buildRunRes.data.name;
        steps3.push(`[3/4] Build run started: ${buildRunName}`);

        // wait for build run
        const buildMaxWait = ((args.build_timeout_seconds as number) || 600) * 1000;
        const buildStart = Date.now();
        let buildLast: any = {};
        const buildPollHistory: Array<{ elapsed_s: number; status: string; reason?: string }> = [];
        while (Date.now() - buildStart < buildMaxWait) {
          ...
          if (buildPollHistory.length === 0 || buildPollHistory[buildPollHistory.length - 1].status !== bsnap.status) buildPollHistory.push(bsnap);
          if (buildLast.status === 'succeeded') break;
        ...
        // wait for app ready
        const appMaxWait = ((args.deploy_timeout_seconds as number) || 180) * 1000;
        const appStart = Date.now();
        let appLast: any = {};
        const appPollHistory: Array<{ elapsed_s: number; status: string; reason?: string; revision?: string }> = [];
        while (Date.now() - appStart < appMaxWait) {
          ...
          if (appPollHistory.length === 0 || appPollHistory[appPollHistory.length - 1].status !== asnap.status) appPollHistory.push(asnap);
          if (appLast.status === 'ready') break;
      ...
      case 'ce_update_build': {
        ...
        const etag = current.data.entity_tag;
        const patch: any = {};
        if (args.output_image) patch.output_image = args.output_image;
        if (args.output_secret) patch.output_secret = args.output_secret;
        if (args.source_url) patch.source_url = args.source_url;
        if (args.source_revision) patch.source_revision = args.source_revision;
        if (args.strategy_size) patch.strategy_size = args.strategy_size;
        if (args.strategy_spec_file) patch.strategy_spec_file = args.strategy_spec_file;
      ...
      case 'ce_list_function_runtimes': {
        const token = await getIAMToken(getApiKey());
        const region = (args.region as string) || 'us-south';
        const response = await axios.get(
          `https://api.${region}.codeengine.cloud.ibm.com/v2/function_runtimes`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        return { content: [{ type: 'text', text: JSON.stringify(response.data, null, 2) }] };
      }
      ...
      case 'ce_create_function': {
        ...
        if (args.env_vars) {
          body.run_env_variables = Object.entries(args.env_vars as Record<string, string>).map(([name, value]) => ({ type: 'literal', name, value }));
        }
      ...
      case 'ce_update_function': {
        ...
        const etag = current.data.entity_tag;
        const patch: any = {};
        if (args.runtime) patch.runtime = args.runtime;
        if (args.code_reference) patch.code_reference = args.code_reference;
        if (args.code_main) patch.code_main = args.code_main;
        if (args.scale_concurrency !== undefined) patch.scale_concurrency = args.scale_concurrency;
        if (args.scale_cpu_limit) patch.scale_cpu_limit = args.scale_cpu_limit;
        if (args.scale_memory_limit) patch.scale_memory_limit = args.scale_memory_limit;
        if (args.env_vars) {
          patch.run_env_variables = Object.entries(args.env_vars as Record<string, string>).map(([name, value]) => ({ type: 'literal', name, value }));
      ...
      case 'ce_create_allowed_outbound_destination': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const body: any = { name: args.name, type: args.type };
        if (args.cidr_block) body.cidr_block = args.cidr_block;
      ...
      case 'ce_update_allowed_outbound_destination': {
        ...
        const etag = current.data.entity_tag;
        const patch: any = {};
        if (args.cidr_block) patch.cidr_block = args.cidr_block;
        if (args.fqdn) patch.fqdn = args.fqdn;
      ...
      case 'ce_create_persistent_data_store': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const body: any = { name: args.name, secret_name: args.secret_name, bucket_name: args.bucket_name };
        if (args.endpoint) body.endpoint = args.endpoint;
      ...
      case 'ce_create_fleet': {
        const token = await getIAMToken(getApiKey());
        const { base, headers } = await ceApi(args.project_id as string, token);
        const body: any = { name: args.name, image_reference: args.image };
        if (args.image_secret) body.image_secret = args.image_secret;
        if (args.scale_cpu_limit) body.scale_cpu_limit = args.scale_cpu_limit;
        if (args.scale_memory_limit) body.scale_memory_limit = args.scale_memory_limit;
        if (args.env_vars) {
