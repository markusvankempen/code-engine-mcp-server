...
const TS = Date.now();
const uniq = (prefix: string) => `${prefix}-${TS}`;
...
function detectRuntime(): string | null {
  for (const rt of ['podman', 'docker']) {
    try { execSync(`which ${rt}`, { stdio: 'ignore' }); return rt; } catch { /* not found */ }
  }
  return null;
}
...
function ok(group: string, tool: string, result: any) {
...
function failWith(group: string, tool: string, err: any) {
  console.log(`  ❌  FAIL  [${group}] ${tool} — ${err?.message || err}`);
  fail++;
}

function sk(group: string, tool: string, reason: string) {
...
function callTool(toolName: string, toolArgs: Record<string, any>): Promise<any> {
  return new Promise((resolve, reject) => {
    const proc: ChildProcess = spawn('node', [SERVER], { stdio: ['pipe', 'pipe', 'pipe'] });
    let stdout = '';
...
async function main() {
  ...
  if (!projectId) { console.error('No project found — set CE_PROJECT_ID'); process.exit(1); }

  const projectData = parseToolResult(await callTool('ce_get_project', { project_id: projectId }));
  const region: string = projectData?.region ?? 'us-south';
  console.log(`  Project: ${projectId}  Region: ${region}`);
  if (RUNTIME) console.log(`  Container runtime: ${RUNTIME}`);
  else console.log('  Container runtime: none detected — container tests will be skipped');
  console.log();
  ...
  } catch (e) { failWith('container', 'build_container_image', e); }
  ...
  if (imageBuildPassed) {
    ...
    } catch (e) { failWith('container', 'test_container_locally', e); }
  } else {
  ...
  if (containerId) {
    ...
    } catch (e) { failWith('container', 'get_container_logs', e); }
  } else {
  ...
  if (containerId) {
    ...
    } catch (e) { failWith('container', 'stop_local_container', e); }
  } else {
  ...
  if (pushImage) {
    ...
    } catch (e) { failWith('container', 'push_container_image', e); }
  } else {
  ...
  } catch (e) { failWith('ce-secret', 'ce_create_secret / ce_get_secret', e); } finally {
    if (secretCreated) {
      try {
        ok('ce-secret', 'ce_delete_secret', await callTool('ce_delete_secret', { project_id: projectId, secret_name: secretName }));
      } catch (e) { failWith('ce-secret', 'ce_delete_secret', e); }
    }
  }
  console.log();
  ...
  } catch (e) { failWith('ce-configmap', 'ce_create_config_map / ce_get_config_map', e); } finally {
    if (cmCreated) {
      try {
      ...
      } catch (e) { failWith('ce-configmap', 'ce_delete_config_map', e); }
    }
  }
  console.log();
  ...
  try {
    ...
    if (buildCreated) {
      ...
      } catch (e) { failWith('ce-build', 'ce_create_build_run', e); } finally {
        if (runCreated) {
          try { ok('ce-build', 'ce_delete_build_run', await callTool('ce_delete_build_run', { project_id: projectId, build_run_name: runName })); }
          catch (e) { failWith('ce-build', 'ce_delete_build_run', e); }
        }
      }
    }
  } catch (e) { failWith('ce-build', 'ce_create_build', e); } finally {
    if (buildCreated) {
      try {
        ok('ce-build', 'ce_delete_build', await callTool('ce_delete_build', { project_id: projectId, build_name: buildName }));
      } catch (e) { failWith('ce-build', 'ce_delete_build', e); }
    }
  }
  console.log();
  ...
  let jobRunName: string | null = null;
  ...
  try {
    ...
    if (jobCreated) {
      ...
      } catch (e) { failWith('ce-job', 'ce_create_job_run', e); }
    }
  } catch (e) { failWith('ce-job', 'ce_create_job', e); } finally {
    if (jobRunCreated && jobRunName) {
      try { ok('ce-job', 'ce_delete_job_run', await callTool('ce_delete_job_run', { project_id: projectId, job_run_name: jobRunName })); }
      catch (e) { failWith('ce-job', 'ce_delete_job_run', e); }
    }
    if (jobCreated) {
      try { ok('ce-job', 'ce_delete_job', await callTool('ce_delete_job', { project_id: projectId, job_name: jobName })); }
      catch (e) { failWith('ce-job', 'ce_delete_job', e); }
    }
  }
  console.log();
  ...
  try {
    ...
    if (appCreated) {
      ...
      if (instances.length > 0) {
        ...
        } catch (e) { failWith('ce-app', 'ce_get_app_logs', e); }
      } else {
        sk('ce-app', 'ce_get_app_logs', 'app scaled to zero — no instances running');
      }
    }
  } catch (e) { failWith('ce-app', 'ce_create_application', e); } finally {
    if (appCreated) {
      try { ok('ce-app', 'ce_delete_application', await callTool('ce_delete_application', { project_id: projectId, app_name: appName })); }
      catch (e) { failWith('ce-app', 'ce_delete_application', e); }
    }
  }
  console.log();
