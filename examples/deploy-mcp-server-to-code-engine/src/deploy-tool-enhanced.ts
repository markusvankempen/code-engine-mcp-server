...
interface EnhancedDeploymentSpec {
  ...
  port?: number;
  region?: string;
  projectName?: string;  // Optional: if not provided, will prompt user
  namespace?: string;    // Optional: if not provided, will use default
  interactive?: boolean; // If true, prompt for missing values
}
 ...
 */
async function selectProjectInteractive(projects: Project[]): Promise<Project> {
 ...
 */
export async function deployWithContext(spec: EnhancedDeploymentSpec): Promise<DeploymentResult> {
  const steps: string[] = [];
  ...
  try {
    ...
    const url = await deployApplication(
      token,
      region,
      selectedProject.id,
      spec.appName,
...
if (isMainModule) {
  ...
  const spec: EnhancedDeploymentSpec = {
    appName: args[0],
    appDir: args[1],
