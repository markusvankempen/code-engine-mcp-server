...
async function getApplicationLogs(
  ...
  try {
    ...
    const appResponse = await axios.get(
      `https://api.${region}.codeengine.cloud.ibm.com/v2/projects/${projectId}/apps/${appName}`,
      { headers: { 'Authorization': `Bearer ${token}` } }
    );

    // Get instances for the application
    const instancesResponse = await axios.get(
      `https://api.${region}.codeengine.cloud.ibm.com/v2/projects/${projectId}/apps/${appName}/instances`,
      { headers: { 'Authorization': `Bearer ${token}` } }
    );
    ...
    if (instancesResponse.data.instances && instancesResponse.data.instances.length > 0) {
      ...
      try {
        const logsResponse = await axios.get(
          `https://api.${region}.codeengine.cloud.ibm.com/v2/projects/${projectId}/apps/${appName}/instances/${instance.name}/logs`,
          { headers: { 'Authorization': `Bearer ${token}` } }
        );
...
export async function deployToCodeEngine(spec: DeploymentSpec): Promise<DeploymentResult> {
  const steps: string[] = [];
  ...
  try {
    ...
    const project = projectsResponse.data.projects?.find((p: any) => p.status === 'active') || 
    ...
    try {
      await axios.post(
        `https://api.${region}.codeengine.cloud.ibm.com/v2/projects/${project.id}/secrets`,
        {
    ...
    try {
      await axios.get(
        `https://api.${region}.codeengine.cloud.ibm.com/v2/projects/${project.id}/apps/${spec.appName}`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      appExists = true;
    } catch (error: any) {
    ...
    if (appExists) {
      ...
      const updateResponse = await axios.patch(
        `https://api.${region}.codeengine.cloud.ibm.com/v2/projects/${project.id}/apps/${spec.appName}`,
        {
          ...
          run_env_variables: [
            ...getResponse.data.run_env_variables.filter((v: any) => v.name !== 'DEPLOY_TIME'),
      ...
      const createResponse = await axios.post(
        `https://api.${region}.codeengine.cloud.ibm.com/v2/projects/${project.id}/apps`,
        {
          name: spec.appName,
          image_reference: imageTag,
